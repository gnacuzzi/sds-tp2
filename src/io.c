...
void write_static_file(const char *filename, int n, double L, double rc, double v, double eta,
                       int leader_mode) {
...
void append_correlation_frame(const char *filename, double c, double t) {
    ...
    if (file == NULL) {
        fprintf(stderr, "Error opening dynamic correlation file %s\n", filename);
        return;
    }
...
void append_mean_angle_frame(const char *filename, double a, double t) {
    ...
    if (file == NULL) {
        fprintf(stderr, "Error opening dynamic mean angle file %s\n", filename);
        return;
    }
