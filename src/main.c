 ...
 */
static void update_particles_no_leader(
    Particle *particles,
...
static void update_particles_fixed_leader(
    Particle *particles,
    Particle *next_particles,
    int n,
    double L,
...
static void update_particles_circular_leader(
    Particle *particles,
    Particle *next_particles,
    int n,
    double L,
...
static void initialize_circular_leader(
    Particle *particles,
    double speed,
    double cx,
    double cy,
...
static void print_usage(const char *program_name) {
    ...
    fprintf(stderr, "  leader_mode = 1  -> lider de direccion fija (todavia no implementado)\n");
...
int main(int argc, char *argv[]) {
    ...
    for (int step = 1; step <= steps; step++) {
        ...
        if (leader_mode == 0) {
            ...
            update_particles_circular_leader(
                particles, next_particles, n, L, speed, dt, eta, neighbors, neighbor_count,
                LEADER_CX_DEFAULT, LEADER_CY_DEFAULT, LEADER_RADIUS_DEFAULT
            );
        }
